...
class shell_transport_task(transport_task):
    ...
    def __init__(self, parent, object_class="VI.Transport.DPR.ShellTransport, VI.Transport.DPR", source_element=None):
        ...
        if source_element is None:
            self.xml_set_attribute("Display", "Synchronization Project Transport")

        self.remap_XML_nodes()
    
    def remap_XML_nodes(self, reassign=False):
        xml_shells = self.xml_get_parameter("Shells")
        self.shells = object_parameter(self, "Shells", source_element=xml_shells)

    def children(self):
        child_entries = self.shells.xml_get_children()
    ...
    def prepare_export_data(self):
        ...
        self.xml_append_node(self.shells)
